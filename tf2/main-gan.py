...
for epoch in range(args.maxiter+1):
    ...
    for _ge in range(args.itg):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as gen_tape:
            ...
            # final loss by weighted averaging 
    ...
    itr_prints_gen = '[Info] Epoch: %05d, gloss: %.2f (mse%.3f, adv%.3f, perc:%.3f), gen_elapse: %.2fs/itr' % (\
                     epoch, gen_loss.numpy().mean(), loss_mse.numpy().mean()*args.lmse, loss_adv.numpy().mean()*args.ladv, \
                     perc_loss.numpy().mean()*args.lperc, (time.time() - time_git_st)/args.itg, )
